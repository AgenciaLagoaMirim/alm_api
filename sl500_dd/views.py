...
class DataReceptionSL500(viewsets.ViewSet):
    ...
    def process_data(self, raw_data):
        if "#" in raw_data:
            ...
            for line in lines:
                ...
                if (
                    len(line_parts) >= 7
                ):  # Garantir que haja pelo menos 7 partes em cada linha de SL500P
    ...
    def create(self, request):
        raw_data = request.data.get(
            "DADOS", ""
        )  # Assuming 'DADOS' is the key for your raw data
        station_name = request.data.get(
            "st_name", None
        )  # Assuming 'STATION_NAME' is the key for the station name
